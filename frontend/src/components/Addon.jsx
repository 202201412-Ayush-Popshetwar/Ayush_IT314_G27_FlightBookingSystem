...
const Addons = ({loggedInUser , setLoggedInUser  }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ backgroundImage: "url('/flight.jpg')", backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}>
      ...
      <Header type="list" />
      <div className="flex w-10/12 mx-auto bg-white shadow-xl rounded-xl p-8 my-10">
        <div className="flex-1">
          <h1 className="text-4xl font-semibold text-gray-800 mb-8 text-center">Enhance Your Travel Experience</h1>
          <div className="space-y-8">
            {addons.map((addon) => (
              <div key={addon.id} className="flex justify-between items-center border-b border-gray-300 pb-6">
      ...
      <div className="w-10/12 mb-8 mx-auto bg-gray-100 p-4 rounded-lg mt-4">
        <h2 className="text-2xl font-bold text-gray-800 mb-4">Selected Addons</h2>
        <ul>
          {addons.map((addon) => {
            const selectedVariety = selectedVarieties[addon.id];
            return (
        ...
        <div className="mt-4 flex justify-between items-center">
          ...
          <button type="button"
              ...
              className="px-8 py-4 bg-blue-700 text-white text-lg font-semibold rounded-lg hover:bg-[#003580] transition duration-300">
            Continue to Payment
          </button>
        </div>
      </div>
